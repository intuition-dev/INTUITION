...
export class DownloadFrag {
   constructor(dir, devops:boolean) {
      ...
      if(!devops) download('https://unpkg.com/mtool-belt@1.3.28/template/headFrag.pug').then(data => {
         fs.writeFileSync(dir+'/headFrag.pug', data)
     })
     if(devops) {
         download('https://unpkg.com/mtool-belt@1.3.28/template/devOps.pug').then(data => {
               fs.writeFileSync(dir+'/devOps.pug', data)
         })
         download('https://unpkg.com/mtool-belt@1.3.28/template/devOps.js').then(data => {
