...
export class DownloadFrag {
   constructor(dir, devops:boolean) {
      ...
      if(!devops) download('https://unpkg.com/setup-depp-defs@1.0.2/template/headFrag.pug').then(data => {
         fs.writeFileSync(dir+'/headFrag.pug', data)
     })
     if(devops) {
         download('https://unpkg.com/setup-depp-defs@1.0.2/template/devOps.pug').then(data => {
               fs.writeFileSync(dir+'/devOps.pug', data)
         })
         download('https://unpkg.com/setup-depp-defs@1.0.2/template/devOps.js').then(data => {
