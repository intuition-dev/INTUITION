...
class DownloadFrag {
    constructor(dir, devops) {
        ...
        if (!devops)
            download('https://unpkg.com/setup-depp-defs@1.0.5/template/headFrag.pug').then(data => {
                fs.writeFileSync(dir + '/headFrag.pug', data);
            });
        if (devops) {
            download('https://unpkg.com/setup-depp-defs@1.0.5/template/devOps.pug').then(data => {
                fs.writeFileSync(dir + '/devOps.pug', data);
            });
            download('https://unpkg.com/setup-depp-defs@1.0.5/template/devOps.js').then(data => {
...
class Comps {
    ...
    process(s, dir, fn) {
        ...
        return new Promise(function (resolve, reject) {
            ...
            let js;
            try {
                js = riotc.compile(s, r_options, fn);
            }
            catch (err) {
                beeper(1);
