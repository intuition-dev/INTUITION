...
class DownloadFrag {
    constructor(dir, devops) {
        ...
        if (!devops)
            download('https://unpkg.com/mtool-belt@1.0.11/template/headFrag.pug').then(data => {
                fs.writeFileSync(dir + '/headFrag.pug', data);
            });
        if (devops) {
            download('https://unpkg.com/mtool-belt@1.0.11/template/devOps.pug').then(data => {
                fs.writeFileSync(dir + '/devOps.pug', data);
            });
            download('https://unpkg.com/mtool-belt@1.0.11/template/devOps.js').then(data => {
