...
return this.editViewModel.getSubDirsList(e).then(e=>{Array.isArray(e)?(e.forEach(e=>{i+='<li><div><i class="i-file"></i>'+e+"</div></li>"}),$(".blog-item.active").append("<ul>"+i+"</ul>")):window.location="/editors"}).then(()=>{$(".blog-item li").each(function(){let e=$(this).text()
e.includes(".md")&&$(this).addClass("md-highlight"),(e.includes(".md")||e.includes(".yaml")||e.includes(".css")||e.includes(".pug")||e.includes(".csv"))&&$(this).addClass("hover-highlight")})})}showMd(e,i){this.editViewModel.getFile(e,i).then(e=>{e?window.myCodeMirror.setValue(e):window.location="/editors"})}saveMd(e,i,t,s){return this.editViewModel.saveFile(e,i,t).then(e=>e).then(()=>{s.removeAttr("disabled"),$(".loader").removeClass("active"),$(".notification").removeClass("d-hide").find(".text").text("The content was successfully updated"),setTimeout(function(){$(".notification").addClass("d-hide").find(".text").text("")},2e3)}).then(()=>{this.compile(e,i,t).then(e=>{if("OK"==e){let e="Files have been built"
