...
var FormViewModel = function() {
 function FormViewModel() {
  this._dataObj = {}, this.dataSourceType = 'real', this.exampleModel = new TabulatorService();
 }
 return FormViewModel.prototype.getViewForm = function(formName) {
  ...
  return this.exampleModel.add(row).then(function() {
   window.location.replace('/screen/example1');
  });
 }, FormViewModel.prototype.update = function(row, cb) {
  return this.exampleModel.update(row, cb).then(function(id) {});
 }, FormViewModel.prototype.delete = function(row) {
  this.exampleModel.delete(row).then(function(id) {
   window.location.replace('/screen/example1');
  });
